...
class UserInfo
{
	...
	{			
        ...
        TypedQuery!(app.system.model.User.User) typedQuery = em.createQuery(criteriaQuery.select(root).where(p1, p2));
		...
		if(user !is null)
		{
			...
			acl_per.addPermission("components.article.list" , "文章列表");
			acl_per.addPermission("components.article.add" , "添加文章");
			acl_per.addPermission("components.article.edit" , "编辑文章");
			acl_per.addPermission("components.article.del" , "删除文章");

			acl_per.addPermission("components.category.list" , "分类列表");
			acl_per.addPermission("components.category.add" , "添加分类");
			acl_per.addPermission("components.category.edit" , "编辑分类");
