    ...
    jQuery(function () {
        ...
        $('.uploder-container').each(function (index) {
            ...
            uploader[index].on('uploadSuccess', function (file, response) {
                ...
                that.find('.fileUpdate').val(nowArray);
                alert('上传成功')
            });
            //可以在这里附加额外上传数据
            ...
            $btns.on('click', 'span', function () {
                ...
                switch (index) {
                    case 0:
                        ...
                        var videoOld = $(this).parents('.uploder-container').find('.fileUpdate')
                        var delateThat = $(this);
                        var videoArray = [];
...
function setArray(that, type, array) {
    if (type == '1') {
        var videoOldArray = that.find('.fileUpdate').val()
    } else if (type == '2') {
        var videoOldArray = that.parents('.uploder-container').find('.fileUpdate').val()
    }
    // 字符串编程数组
    // var videoArray = [];
    if (videoOldArray) {
